...
bool IGFX::processKext(KernelPatcher &patcher, size_t index, mach_vm_address_t address, size_t size) {
	...
	if ((currentFramebuffer && currentFramebuffer->loadIndex == index) ||
		...
		if (blackScreenPatch) {
			...
			// for IceLake
			if (!foundSymbol) {
				KernelPatcher::RouteRequest request("__ZN14AppleIntelPort16computeLaneCountEPK29IODetailedTimingInformationV2jjPj", wrapComputeLaneCount, orgComputeLaneCount);
				patcher.routeMultiple(index, &request, 1, address, size);
			}
		}
